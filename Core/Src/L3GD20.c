...
void L3GD20_Init(void)
{
	...
	L3GD4200_Write_Data(CTRL_REG1, 0x0F);
	L3GD4200_Write_Data(CTRL_REG2, 0x00);
	L3GD4200_Write_Data(CTRL_REG3, 0x08);
	L3GD4200_Write_Data(CTRL_REG4, 0x80);
	L3GD4200_Write_Data(CTRL_REG5, 0x00);
...
// 	HAL_SPI_Init(&hspi5);
// 	HAL_SPI_Transmit(&hspi5, spiTxBuf, 2, 50);
// 	while (hspi5.State == HAL_SPI_STATE_BUSY)
...
// 	}
// }
uint8_t L3GD4200_Read_Reg(uint8_t REG_Address)
{
	uint8_t REG_Data;
	HAL_GPIO_WritePin(SPI5_CS1_GPIO_Port, SPI5_CS1_Pin, 0);
	REG_Address |= 0x80;
	HAL_SPI_TransmitReceive(&hspi5, &REG_Address, &REG_Data, 2, 50);
	HAL_GPIO_WritePin(SPI5_CS1_GPIO_Port, SPI5_CS1_Pin, 1);
	return REG_Data;
}

void L3GD4200_Write_Data(uint8_t REG_Address, uint8_t REG_Data)
{
	uint8_t arrTxbuf[2];
	arrTxbuf[0] = REG_Address & 0x7f;
	arrTxbuf[1] = REG_Data;
	HAL_GPIO_WritePin(SPI5_CS1_GPIO_Port, SPI5_CS1_Pin, 0);
	HAL_SPI_Transmit(&hspi5, arrTxbuf, 2, 50);
	HAL_GPIO_WritePin(SPI5_CS1_GPIO_Port, SPI5_CS1_Pin, 1);
}